...
inline GsTLCoord squared(const GsTLCoord x) {
  return(x*x);
}
...
inline void swap(GsTLCoord& a, GsTLCoord&b) {
  GsTLCoord tmp;
  tmp = a;
  a = b;
  b = tmp; 
...
// 
GsTLCoord kdtree2_result_vector::max_value() {
...
GsTLCoord kdtree2_result_vector::replace_maxpri_elt_return_new_maxpri(kdtree2_result& e) {
...
/*
//Add Alexandre Boucher
void kdtree2::set_informatation_property() {
  prop_ = prop;
  node_id_ = node_id;
}
*/
...
kdtree2_node* kdtree2::build_tree_for_range(int l, int u, kdtree2_node* parent) {
  ...
  kdtree2_node* node = new kdtree2_node(dim, prop_, node_id_);
  ...
  if ((u-l) <= bucketsize) {
	...
	node->box[i] = parent->box[i];
      ...
      GsTLCoord spread = node->box[i].upper - node->box[i].lower; 
      if (spread>maxspread) {
	maxspread = spread;
	c=i; 
      }
	...
	average = sum / static_cast<GsTLCoord> (u-l+1);
      } else {
...
void kdtree2:: spread_in_coordinate(int c, int l, int u, interval& interv)
{
  ...
  if (i == u+1) {
    GsTLCoord last = the_data[ind[u]] [c];
...
int kdtree2::select_on_coordinate_value(int c, GsTLCoord alpha, int l, int u) {
  //
...
static const GsTLCoord infinity = 1.0e38;
...
public:
  searchrecord(vector<GsTLCoord>& qv_in, kdtree2& tree_in,
...
void kdtree2::n_nearest_brute_force(vector<GsTLCoord>& qv, int nn, kdtree2_result_vector& result) {
...
void kdtree2::n_nearest(vector<GsTLCoord>& qv, int nn, kdtree2_result_vector& result) {
  searchrecord sr(qv,*this,result);
  vector<GsTLCoord> vdiff(dim,0.0); 
...
void kdtree2::n_nearest_around_point(int idxin, int correltime, int nn,
  ...
  vector<GsTLCoord> qv(dim);  //  query vector
...
void kdtree2::r_nearest(vector<GsTLCoord>& qv, GsTLCoord r2, kdtree2_result_vector& result) {
...
int kdtree2::r_count(vector<GsTLCoord>& qv, GsTLCoord r2) {
// search for all within a ball of a certain radius
  {
    kdtree2_result_vector result; 
    searchrecord sr(qv,*this,result);
...
void kdtree2::r_nearest_around_point(int idxin, int correltime, GsTLCoord r2,
				     kdtree2_result_vector& result) {
  vector<GsTLCoord> qv(dim);  //  query vector
...
int kdtree2::r_count_around_point(int idxin, int correltime, GsTLCoord r2) 
{
  vector<GsTLCoord> qv(dim);  //  query vector
...
inline GsTLCoord dis_from_bnd(GsTLCoord x, GsTLCoord amin, GsTLCoord amax) {
  if (x > amax) {
    return(x-amax); 
  } else if (x < amin)
    return (amin-x);
...
inline bool kdtree2_node::box_in_search_range(searchrecord& sr) {
  ...
  int dim = sr.dim;
  GsTLCoord dis2 =0.0; 
  GsTLCoord ballsize = sr.ballsize; 
  for (int i=0; i<dim;i++) {
...
/*
Modified Alexandre Boucher.
A terminal node only need to be consider if it is informed.
...
void kdtree2_node::process_terminal_node(searchrecord& sr) {
  ...
  for (int i=l; i<=u;i++) {
    int indexofi= sr.ind[i];   // A boucher: get the index straight from the beginning, see comment below
    if( !prop_->is_informed( (*node_id_)[indexofi]  )  || !prop_->is_inside_region((*node_id_)[indexofi] ) ) continue;  //(*node_id_)[indexofi]
    GsTLCoord dis;
    bool early_exit; 
    ...
    if (rearrange) {
      ...
      //
      // Add Boucher: we need the indices up front to check if the node is informed.  That
      // line is moved at the beginning of the for loop.
      //indexofi = sr.ind[i];
    } else {
      // 
...
void kdtree2_node::process_terminal_node_fixedball(searchrecord& sr) {
  ...
  for (int i=l; i<=u;i++) {
    ...
    if( !prop_->is_informed( (*node_id_)[indexofi] ) )  continue;           //(*node_id_)[indexofi] ) ) continue;
    GsTLCoord dis;
    bool early_exit; 
