 ...
 *  MgridNeighborhood
 **************************************************/
BOOST_AUTO_TEST_CASE(MgridNeighborhood___find_neighbors)
{
  std::string grid_name("grid, MgridNeighborhood___find_neighbors");
  ...
  std::string prop_name("prop, MgridNeighborhood___find_neighbors");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
  
  // create neighborhood
  MgridNeighborhood * nbh = six_face_mgrid_neighborhood(grid, prop);
  BOOST_REQUIRE(nbh);
...
BOOST_AUTO_TEST_CASE(MgridNeighborhood___find_neighbors___4threads)
{
  std::string grid_name("grid, MgridNeighborhood___find_neighbors___4threads");
  Reduced_grid* grid = reduced_grid(grid_name);
  BOOST_REQUIRE(grid);
  std::vector<bool> mask(3*3*3, true);
  grid->set_dimensions(3, 3, 3, 1, 1, 1, mask);
  ...
  // add property to grid
  std::string prop_name("prop, MgridNeighborhood___find_neighbors___4threads");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
...
BOOST_AUTO_TEST_CASE(MgridNeighborhood___center)
{
  std::string grid_name("grid, MgridNeighborhood___center");
  Reduced_grid* grid = reduced_grid(grid_name);
  BOOST_REQUIRE(grid);
  std::vector<bool> mask(3*3*3, true);
  grid->set_dimensions(3, 3, 3, 1, 1, 1, mask);
  ...
  // add property to grid
  std::string prop_name("prop, MgridNeighborhood___center");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
 ...
 **************************************************/
BOOST_AUTO_TEST_CASE(MgridNeighborhood_hd___find_neighbors)
{
  std::string grid_name("grid, MgridNeighborhood_hd___find_neighbors");
  Reduced_grid* grid = reduced_grid(grid_name);
  BOOST_REQUIRE(grid);
  std::vector<bool> mask(3*3*3, true);
  grid->set_dimensions(3, 3, 3, 1, 1, 1, mask);
  ...
  // add property to grid
  std::string prop_name("prop, MgridNeighborhood_hd___find_neighbors");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
  
  // create neighborhood
  MgridNeighborhood_hd * nbh = six_face_mgrid_neighborhood_hd(grid, prop);
  BOOST_REQUIRE(nbh);
  BOOST_REQUIRE(nbh->geometry().size() == 6);
  ...
  nbh->find_neighbors(grid->geovalue(node_id));
  BOOST_CHECK( two_geovalue_lists_equal(nbh->begin(), nbh->end(), neighbors.begin(), neighbors.end()) );
  // 1 neighbors harddata
  grid->geovalue(cursor.node_id(1, 0, 0)).set_harddata(true);
  node_id = cursor.node_id(0, 0, 0);
  nbh->find_neighbors( grid->geovalue(node_id), neighbors);
  ...
  nbh->find_neighbors(grid->geovalue(node_id));
  BOOST_CHECK( two_geovalue_lists_equal(nbh->begin(), nbh->end(), neighbors.begin(), neighbors.end()) );
  // 2 neighbors harddata
  grid->geovalue(cursor.node_id(0, 1, 0)).set_harddata(true);
  grid->geovalue(cursor.node_id(1, 1, 1)).set_harddata(true);
  node_id = cursor.node_id(0, 0, 0);
  nbh->find_neighbors( grid->geovalue(node_id), neighbors);
  BOOST_CHECK(neighbors.number_harddata_neighbors() == 2);
...
BOOST_AUTO_TEST_CASE(MgridNeighborhood_hd___find_neighbors___4threads)
{
  std::string grid_name("grid, MgridNeighborhood_hd___find_neighbors___4threads");
  Reduced_grid* grid = reduced_grid(grid_name);
  BOOST_REQUIRE(grid);
  std::vector<bool> mask(3*3*3, true);
  grid->set_dimensions(3, 3, 3, 1, 1, 1, mask);
  ...
  // add property to grid
  std::string prop_name("prop, MgridNeighborhood_hd___find_neighbors___4threads");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
...
BOOST_AUTO_TEST_CASE(MgridNeighborhood_hd___center)
{
  std::string grid_name("grid, MgridNeighborhood_hd___center");
  Reduced_grid* grid = reduced_grid(grid_name);
  BOOST_REQUIRE(grid);
  std::vector<bool> mask(3*3*3, true);
  grid->set_dimensions(3, 3, 3, 1, 1, 1, mask);
  ...
  // add property to grid
  std::string prop_name("prop, MgridNeighborhood_hd___center");
  Grid_continuous_property * prop = grid->add_property(prop_name);
  BOOST_REQUIRE(prop);
  grid->select_property(prop_name);
