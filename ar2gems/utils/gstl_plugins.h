...
#define BIND_ACTION_COMMAND(CLASSNAME,ACTIONNAME) \
{\
  GsTLlog << "\n\n registering filter #CLASSNAME" << "\n"; \
  SmartPtr<Named_interface> ni = \
  ...
  CLASSNAME toto; \
  dir->factory( ACTIONNAME, CLASSNAME##::create_new_interface ); \
} \


#if defined(_WIN32) || defined(WIN32)
#define INPUT_ACTION_PLUGIN(CLASSNAME, ACTIONNAME)\
extern "C" __declspec(dllexport) int plugin_init() { \
  BIND_ACTION_COMMAND(CLASSNAME, ACTIONNAME)\
  return 0; \
}\
