...
void SimplePs::drawCurve(QwtData & data,int style)
{
	drawCurve(data,style,false);
}

void SimplePs::drawCurve(QwtData & data,int style, bool right)
{
	int i;
	...
	xmin = _plot->axisScaleDiv(QwtPlot::xBottom)->lBound();
	...
	if (my.transformation()->type() == QwtScaleTransformation::Log10){
		for (i = 0; i < y.size(); ++i)
			if (y[i] > 0)
				y[i] = log10(y[i]);
		ymin = log10(ymin);
	...
	else if (style == QwtPlotCurve::Dots) {
		for (i = 0; i < x.size(); ++i) {
			x1 = (x[i]-xmin)*xscale;
			y1 = (y[i]-ymin)*yscale;
	...
	else if (style == QwtPlotCurve::Histogram) {
		float x1,x2=0,y2;
		for ( i = 0;i < x.size(); ++i) {
			x1 = (x[i]-xmin)*xscale;
			y2 = (y[i]-ymin)*yscale;
