...
void Lib_initializer::release()
{
	...
	Root::instance()->delete_interface(colorscale_manager);
...
{
	SmartPtr<Named_interface> ni = Root::instance()->interface(colorscale_manager);
	Manager* mng = dynamic_cast<Manager*> (ni.raw_ptr());
	appli_assert( mng );
	...
	for (; it != list.end(); ++it)
	{
		...
		Root::instance()->new_interface("colorscale://" + std::string(s1.constData()), colorscale_manager + "/" + std::string(s2.constData()));
	}
}
