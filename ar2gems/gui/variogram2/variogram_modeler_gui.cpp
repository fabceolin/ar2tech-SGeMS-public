...
Variogram_modeler_gui::recompute_variograms() {
  this->setFocus(Qt::OtherFocusReason);  // Get the focus to ensure that the entry are all read
...
compute_df_pset( std::vector<Discrete_function> &df,
  ...
  for( int i2=0 ; i2 < angle_table->rowCount() ; i2++ ) {
    ...
    if(!angle_table->item(i2,0)->text().isEmpty()) { 
      ...
      if(angle_table->item(i2,0) == 0) {
        GsTLcerr << "No azimuth angle entered" << gstlIO::end;
        return false;      
      }

      if( angle_table->item(i2,1) == 0 ) { // ->data(Qt::DisplayRole).isNull() ){ //.canConvert<QString>()) {
        GsTLcerr << "No dip angle entered" << gstlIO::end;
        return false;      
      }
      if(angle_table->item(i2,2) == 0) {
        GsTLcerr << "No angle tolerance entered" << gstlIO::end;
        return false;      
      }
      if(angle_table->item(i2,3) == 0) {
        GsTLcerr << "No bandwidh entered" << gstlIO::end;
        return false;      
      }
