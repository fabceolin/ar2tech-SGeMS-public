 ...
 : QWidget(parent), id_(id), vtk_prop_(vtk_prop)
{
  ...
  QLabel* section_orientation = new QLabel(orientation, this);
	...
	section_spin_->setRange(0,max_extent);
	section_spin_->setValue(0);
	QSlider* section_slider = new QSlider(Qt::Horizontal, this);
	section_slider->setRange(0,max_extent);
	section_slider->setValue(0);
...
Property_section_view::
Property_section_view(const GsTLGridProperty* prop,GsTL_vtkProp* vtk_prop,QWidget *parent)
 : QScrollArea(parent), prop_(prop),vtk_prop_(vtk_prop), section_counter_(0){
...
Property_section_view::~Property_section_view(){}

int Property_section_view::number_of_enabled_sections(void){
	int n = 0;
...
int Property_section_view::number_of_sections(void){
	return sections_.size();
}


bool Property_section_view::is_section_enabled(){
	for(int i=0;i<sections_.size(); ++i) {
...
void Property_section_view::add_section(){
...
void Property_section_view::remove_section(Single_section_widget* section){
...
void Property_section_view::display(){


  emit this->rendering_modified();
}
void Property_section_view::undisplay(){
  emit this->rendering_modified();
	//vtk_prop_->clear_thresholding();
}


void Property_section_view::select_all_sections(){
	for(int i=0; i < sections_.size() ; ++i ) {
		sections_.at(i)->set_enabled(true);
	}
  emit this->rendering_modified();
}
void Property_section_view::unselect_all_sections(){
	for(int i=0; i < sections_.size() ; ++i ) {
