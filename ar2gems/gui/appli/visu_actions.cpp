				  ...
				  GsTL_project* proj, Error_messages_handler* errors ) 
{
	...
	Vtk_view * dir1 = dynamic_cast<Vtk_view*>( ni.raw_ptr() );
	appli_assert( dir1 );
...
init( std::string& parameters, GsTL_project* proj,
  ...
  SmartPtr<Named_interface> view_ni =
    Root::instance()->interface( projectViews_manager + "/main_view"  );
  Vtk_view* view = dynamic_cast<Vtk_view*>( view_ni.raw_ptr() );
  if( !view ) {
    return false;
  }
...
init( std::string& parameters, GsTL_project* proj,
  ...
  view->get_render_area()->parentWidget()->resize( width, height );
  return true;
}
...
bool Display_object::init( std::string& parameters, GsTL_project* proj,
  ...
  if( params.size() == 1 ) {
	  Visualization_parameters* viz_param = view->get_viz_parameters( grid);
...
bool Snapshot_action::init( std::string& parameters, GsTL_project* proj,
  ...
  Snapshot_dialog snap( view->get_render_area()->GetRenderWindow() );
