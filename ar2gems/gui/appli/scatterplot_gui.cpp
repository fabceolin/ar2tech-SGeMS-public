...
Scatterplot_gui::Scatterplot_gui( GsTL_project* project,
  ...
  : Data_analysis_gui( parent, name ),
    regression_line_curve_id_( NULL ) {

  setWindowTitle( "Scatterplot" );
  ...
  control_panel_ = 
    new Scatterplot_control_panel( project, scrollview_->viewport(),
                                   "control_panel" );
  scrollview_->setWidget( control_panel_ );
  ...
  QwtSymbol sym( QwtSymbol::Ellipse, QBrush( Qt::black ), QPen( Qt::black ),
                 QSize( 6,6 ) );
  curve_id_->setSymbol(sym);
  curve_id_->setStyle(QwtPlotCurve::Dots);
...
void Scatterplot_gui::draw_regression_line( bool ok ) {
  ...
  if (!regression_line_curve_id_)
    regression_line_curve_id_ = new QwtPlotCurve();
  ...
  float xmin = plot_->axisScaleDiv( QwtPlot::xBottom )->lBound();
  float xmax = plot_->axisScaleDiv( QwtPlot::xBottom )->hBound();
  float ymin = plot_->axisScaleDiv( QwtPlot::yLeft )->lBound();
  float ymax = plot_->axisScaleDiv( QwtPlot::yLeft )->hBound();
  ...
  //regression_line_curve_id_ = new QwtPlotCurve();
  regression_line_curve_id_->attach(plot_);


  regression_line_curve_id_->setData( x_curve, y_curve, size );
  regression_line_curve_id_->setPen( QPen( Qt::red ) );
  plot_->replot();
