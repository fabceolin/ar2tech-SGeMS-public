...
Histogram_proxy_model::Histogram_proxy_model(QObject *parent)
...
Histogram_proxy_model::Histogram_proxy_model(QList< GsTL_object_item*> items, QObject *parent)
 : QAbstractItemModel(parent),current_id_(0) {
	grid_manager_ = dynamic_cast<Manager*>(Root::instance()->interface(gridModels_manager).raw_ptr());
...
Histogram_proxy_model::~Histogram_proxy_model() {
...
void Histogram_proxy_model::set_connections(){
  	bool ok;
...
int Histogram_proxy_model::columnCount(const QModelIndex &/*parent*/) const
{
    return 4;
}

Qt::ItemFlags Histogram_proxy_model::flags(const QModelIndex &index) const
{
    if (!index.isValid())
        return 0;
...
bool Histogram_proxy_model::insert_row(Grid_continuous_property* prop, Grid_region* region, Grid_continuous_property* weights, QColor color){
  ...
  Histogram_item* item = new Histogram_property_item(prop);
  item->weights(weights);
  item->region(region);
  item->color(color);

  std::set<Histogram_item*>::iterator it_insert = std::lower_bound(items_.begin(), items_.end(),item);
...
bool Histogram_proxy_model::insert_row(GsTLGridPropertyGroup* group, Grid_region* region, Grid_continuous_property* weights, QColor color){
  ...
  Histogram_item* item = new Histogram_group_item(group);
  item->weights(weights);
  item->region(region);
  item->color(color);
...
bool Histogram_proxy_model::insert_row(Histogram_item* item){
  std::set<Histogram_item*>::iterator it_insert = std::lower_bound(items_.begin(), items_.end(),item);
  unsigned int row = std::distance(items_.begin(), it_insert);
...
bool Histogram_proxy_model::insert_row(Continuous_distribution* dist,QColor color){
  Histogram_item* item = new Histogram_distribution_item(dist, current_id_);
  current_id_++;
  item->color(color);
...
bool Histogram_proxy_model::insert_row(Grid_continuous_property* prop, Grid_weight_property* weights, QColor color){
...
bool Histogram_proxy_model::insert_rows(std::vector<GsTL_object_item*> new_object_items){

  bool ok = false;
  for(int i=0; i<new_object_items.size(); ++i) {
//    std::map<GsTL_object_item*,Histogram_item*>::iterator it =  lookup_items_.find(new_object_items[i]);
...
/*
    ...
    std::set<Histogram_item*>::iterator it_insert = std::lower_bound(items_.begin(), items_.end(),new_item);
    unsigned int row = std::distance(items_.begin(), it_insert);
...
bool Histogram_proxy_model::remove_row(Histogram_item* item){
    std::set<Histogram_item*>::iterator it = items_.find(item);
...
/*
bool Histogram_proxy_model::remove_rows(std::vector<GsTL_object_item*> new_object_items){

  for(int i=0; i<new_object_items.size(); ++i) {
    std::map<GsTL_object_item*,Histogram_item*>::iterator it =  lookup_items_.find(new_object_items[i]);
    if( it != lookup_items_.end() ) continue; // not present

    std::set<Histogram_item*>::iterator it_set = items_.find(it->second);

    unsigned int row = std::distance(items_.begin(), it_set);
    this->beginRemoveRows(QModelIndex() ,row,row );  //All the removal are at the top 
    Histogram_item* item_to_be_removed = it->second;
...
QModelIndex Histogram_proxy_model::index(int row, int column, const QModelIndex &parent)
            const
{
  ...
  if(!parent.isValid()) {
    ...
    Histogram_item* item = static_cast<Histogram_item*>(parent.internalPointer());
...
int Histogram_proxy_model::rowCount(const QModelIndex &parent) const
{
...
QModelIndex Histogram_proxy_model::parent(const QModelIndex &child) const
{
  Histogram_item* item = static_cast<Histogram_item*>(child.internalPointer());
  Histogram_item* parent_item = item->parent();
  if( parent_item == 0 ) return QModelIndex();
  
  std::set<Histogram_item*>::const_iterator it = items_.find(parent_item);
  ...
  //Find the row number 
  //QList<Histogram_item*>::const_iterator it = qBinaryFind(items_.begin(),items_.end(), parent_item, histogram_item_less_than);
}
...
QVariant
Histogram_proxy_model::headerData(int section, Qt::Orientation orientation, int role) const
{
...
QVariant Histogram_proxy_model::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
    ...
    if( role == Qt::DisplayRole ) {
      ...
      else if(index.column() == 3) return  item->display_format();
    }
...
bool Histogram_proxy_model::setData ( const QModelIndex & index, const QVariant & value, int role ){
  bool ok = false;
  if( role == Qt::EditRole ) {
      Histogram_item* item = static_cast<Histogram_item*>(index.internalPointer());
      if(index.column() == 1) {
...
/*
QModelIndex	Histogram_proxy_model::mapFromSource ( const QModelIndex & sourceIndex ) const{
  ...
  std::map<GsTL_object_item*,Histogram_item*>::const_iterator it = lookup_items_.find(object_item);

  if(it == lookup_items_.end()) return QModelIndex();

  Histogram_item* item = it->second;

  // check if the item is a top item or a child of a group
  if(item->parent() == 0) {
    std::set<Histogram_item*>::const_iterator it = items_.find(item);
    if(it == items_.end()) return QModelIndex();
    ...
    Histogram_group_item* parent = dynamic_cast<Histogram_group_item*>(item->parent());
...
QModelIndex	Histogram_proxy_model::mapToSource ( const QModelIndex & proxyIndex ) const{
...
/*
void Histogram_proxy_model::begin_insert_proxy_rows(const QModelIndex & source_parent, int start, int end){
...
void Histogram_proxy_model::begin_remove_proxy_rows(const QModelIndex & source_parent, int start, int end){

  //Check if the data is currently loaded
  // A property, a region or a set of weights can be removed;  Need to check for each
  
  //std::vector<Histogram_item*> items_to_remove;
  std::set<Histogram_item*> items_to_remove;
 ...
 //   items_to_remove.push_back( lookup_items_[item] );
  ...
  std::set<Histogram_item*>::iterator it = items_to_remove.begin();
...
void Histogram_proxy_model::find_items_to_be_removed(GsTL_object_item* item, std::set< Histogram_item*>& items_to_be_removed){
...
bool Histogram_proxy_model::is_item_exist(Grid_continuous_property* prop, Grid_weight_property* weights){
  std::set< Histogram_item*>::iterator it = items_.begin();
  for( ; it!= items_.end(); ++it) {
    Histogram_property_item* p_item = dynamic_cast<Histogram_property_item*>(*it);
...
/*
void Histogram_proxy_model::end_insert_proxy_rows(const QModelIndex & source_parent, int start, int end){
...
*/
void Histogram_proxy_model::end_remove_proxy_rows(const QModelIndex & source_parent, int start, int end){
  this->endResetModel();
}
