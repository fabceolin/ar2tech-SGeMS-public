...
// debug mode
Named_interface* Simulacre_input_filter::read( const std::string& filename, 
                                           std::string* errors ) {
  QFile file( filename.c_str() );
...
// Read the mask
  ...
  std::string final_grid_name;
	SmartPtr<Named_interface> ni = 
		Root::instance()->new_interface( "reduced_grid://" + name+"::"+ostr.str(),
		gridModels_manager + "/" + grid_name,
		&final_grid_name );
	*/
// Abort if a grid with the same name already exist
	SmartPtr<Named_interface> ni =
		Root::instance()->new_interface( "reduced_grid://" + grid_name+"::"+size_str.str(),
		gridModels_manager + "/" + grid_name );
...
Simulacre_input_filter::read_cartesian_grid( QDataStream& stream, 
  ...
  std::string final_grid_name;
  SmartPtr<Named_interface> ni = 
    Root::instance()->new_interface( "cgrid://" + grid_name, gridModels_manager + "/" + grid_name,
                                     &final_grid_name );
  */
  // Abort if the name already exist
  SmartPtr<Named_interface> ni =
    Root::instance()->new_interface( "cgrid://" + grid_name, gridModels_manager + "/" + grid_name);
...
Simulacre_input_filter::read_pointset( QDataStream& stream, 
  ...
  size_str << size;
  SmartPtr<Named_interface> ni = 
    Root::instance()->new_interface( "point_set://" + grid_name+"::"+size_str.str(),
                                     gridModels_manager + "/" + grid_name);
...
bool Simulacre_output_filter::write( std::string outfile, const Named_interface* ni_grid, 
                                     std::string* errors ) {

  const Geostat_grid* grid = dynamic_cast<const Geostat_grid*>(ni_grid);
