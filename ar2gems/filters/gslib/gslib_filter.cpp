...
Named_interface* Gslib_infilter::read( const std::string& filename, 
                                    std::string* errors ) {
...
Named_interface* Gslib_specialized_infilter::read( const std::string& filename, 
                                                std::string* errors ) {
...
//  bool is_jk = dialog_->is_ijk();
  ...
  Geostat_grid::location_type max_xyz_rotated(-1,-1,-1);
...
//		loc[2] = String_Op::to_number<float>(buf[Z_col_id]);
  ...
  //get the dimension corrected for the angles
  /*
...
Geostat_grid* Gslib_mgrid_infilter::readPointsetFormat(std::ifstream& infile, Reduced_grid * grid)
{
	...
	float y_size = dialog_->y_size();
	float z_size = dialog_->z_size();
  float z_rotation = dialog_->rotation_z();
...
Geostat_grid* Gslib_grid_infilter::read( std::ifstream& infile ) {
  ...
  float Oy = dialog_->Oy();
  float Oz = dialog_->Oz();
  float rotation_z = dialog_->rotation_z();
...
Geostat_grid* Gslib_poinset_infilter::read( std::ifstream& infile ) {
  ...
  ni = Root::instance()->new_interface( "point_set://" + name+"::"+size_str,
					gridModels_manager + "/" + name );
...
bool Gslib_outfilter::write( std::string outfile, const Named_interface* ni, 
                            std::string* errors ) 
{
