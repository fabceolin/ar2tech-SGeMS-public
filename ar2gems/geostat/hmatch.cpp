...
bool HMatch::readMemberVars(const Parameters_handler* parameters)
{
	...
	if (_useSgemsAlgo) {
	  QByteArray tmp = dynamic_cast<const Parameters_handler_xml*>(parameters)->getDoc().toString().toLatin1();
	  parg = Py_BuildValue("(ss)", tmp.constData(),parameters->value("_sgemsAlgo.value").c_str());
	}
	else {
	  QByteArray tmp = dynamic_cast<const Parameters_handler_xml*>(parameters)->getDoc().toString().toLatin1();
	  parg = Py_BuildValue("(ss)", tmp.constData(),"");
	}
...

{
	...
	double overallObj,p,q,r,etemp,d=0.0;
