...
{
  /*
  //Check first is the dev is informed
  bool dev_is_informed = false;
  
  //typename pixel_pattern_type::value_type::const_iterator it = pattern.begin()->begin();
  std::vector<float> test = pattern[0];
  for(int i=0; i < test.size(); ++i) {
    if( !boost::math::isnan(pattern[0][i]) ) {
      dev_is_informed = true;
      break;
    }
  }
  ...
  if(!dev_is_informed) {
    ...
    //Rand48_generator gen;
    int prototype_id = floor( gen()*nb_parent_proto_ ); 

    ListItr itr = prototypes_.begin();
    std::advance(itr,prototype_id );
    return *itr;
  ...
  */
    ...
    /*
    // Check to see if a prototype was found
    // This is a very ugly hack, it should be chosen randomly
    if( dist[prototype_id] == -1 ) {
      int n=6e10;
      for( itr = prototypes_.begin(); itr != prototypes_.end() ; ++itr ) {
        if(itr->)
      }
    }
    */
