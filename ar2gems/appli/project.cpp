...
void GsTL_project::update( std::string obj ) {
  ...
  for( ; it != views_list_.end() ; ++it )
	...
	Manager::interface_iterator it_model_mng = mng->begin_interfaces();
	for( ; it_model_mng != mng->end_interfaces(); ++it_model_mng) {
		GsTL_ItemModel* model = dynamic_cast<GsTL_ItemModel*>(it_model_mng->raw_ptr());
		if(model == 0) continue;
		model->interface_changed();
//		model->reset();
	}
}
...
void begin_add_item(std::string type_hierarchy, std::string  type_name){
	...
	for( ; it != mng->end_interfaces(); ++it) {
		...
		if(model == 0) continue;
		model->begin_add_item(type_hierarchy,type_name);
	}
}
...
void begin_remove_item(std::string type_hierarchy, std::string type_name){
	...
	for( ; it != mng->end_interfaces(); ++it) {
		GsTL_ItemModel* model = dynamic_cast<GsTL_ItemModel*>(it->raw_ptr());
		if(model == 0) continue;
		model->begin_remove_item(type_hierarchy,type_name);
	}
}
...
void begin_update_item(std::string type_hierarchy, std::string type_name){
	...
	for( ; it != mng->end_interfaces(); ++it) {
		GsTL_ItemModel* model = dynamic_cast<GsTL_ItemModel*>(it->raw_ptr());
		if(model == 0) continue;
		model->begin_update_item(type_hierarchy,type_name);
	}
}
...
void end_add_item(std::string type_hierarchy, std::string  type_name){
	...
	for( ; it != mng->end_interfaces(); ++it) {
		GsTL_ItemModel* model = dynamic_cast<GsTL_ItemModel*>(it->raw_ptr());
		if(model == 0) continue;
		model->end_add_item(type_hierarchy,type_name);
	}
}
...
void end_remove_item(std::string type_hierarchy, std::string type_name){
	...
	for( ; it != mng->end_interfaces(); ++it) {
		GsTL_ItemModel* model = dynamic_cast<GsTL_ItemModel*>(it->raw_ptr());
		if(model == 0) continue;
		model->end_remove_item(type_hierarchy,type_name);
	}
}
